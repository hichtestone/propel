...
class PropelMigrationManager
{
    ...
    protected $migratedFilesTable = 'migrated_files';
    protected $migrationDir;
    public $migratedFiles = false;
     ...
     * @return string
     */
    public function getMigratedFilesTable()
    {
        return $this->migratedFilesTable;
    }
    ...
    public function getOldestDatabaseVersion()
    {
        ...
        foreach ($connections as $name => $params) {
			...
			$sql = sprintf('SELECT version FROM %s', $this->getMigratedFilesTable());
			$stmt = $pdo->prepare($sql);
			try {
				$stmt->execute();
				$this->migratedFiles = $stmt->fetchAll(PDO::FETCH_COLUMN, 0);
				return true;
    ...
    public function migratedFilesTableExists($datasource)
    {
    ...
    public function updateLatestMigrationTimestamp($datasource, $timestamp)
    {
        ...
        $stmt->execute();
        if( $this->migratedFilesTableExists($datasource) )
        {
			$sql = sprintf('INSERT IGNORE %s (%s) VALUES (?)',
				$this->getMigratedFilesTable(),
				$platform->quoteIdentifier('version')
			);
			$stmt = $pdo->prepare($sql);
			$stmt->bindParam(1, $timestamp, PDO::PARAM_INT);
	...
	public function removeMigratedFileFromTable($datasource, $nextMigrationTimestamp)
	{
        if( $this->migratedFilesTableExists($datasource) )
        {
			$platform = $this->getPlatform($datasource);
			$pdo = $this->getPdoConnection($datasource);
			$sql = sprintf('DELETE FROM %s where version = ?',
				$this->getMigratedFilesTable()
			);
			$stmt = $pdo->prepare($sql);
			$stmt->bindParam(1, $nextMigrationTimestamp, PDO::PARAM_INT);
			$stmt->execute();
		}		
	}
    ...
    public function getValidMigrationTimestamps()
    {
		...
		if( $this->migratedFiles !== false )
		{
			foreach ($migrationTimestamps as $key => $timestamp) {
				if(in_array($timestamp, $this->migratedFiles)) unset($migrationTimestamps[$key]);
			}
		}
		else
		{
        ...
        sort($migrationTimestamps);
       // var_dump($migrationTimestamps); exit;
    ...
    public function getAlreadyExecutedMigrationTimestamps()
    {
        ...
        $migrationTimestamps = $this->getMigrationTimestamps();
  		...
  		if( $this->migratedFiles !== false ) $oldestMigrationTimestamp = array_pop($this->migratedFiles);
